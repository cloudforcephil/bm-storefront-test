...
- Mark a Page Designer region as [`<Region critical>`](README-PAGE-DESIGNER.md#critical-page-regions) only when its content is required for the initial HTML or is an LCP candidate. Await that page in the route loader. SSR prepares direct children as each critical region is actually rendered and emits resource hints only for component wrappers it reaches; conditional nested payloads are not speculatively loaded. Component loader data keeps its granular Suspense boundaries. Do not mark below-the-fold or broad catch-all regions critical: doing so delays the shell without improving the initial viewport.
